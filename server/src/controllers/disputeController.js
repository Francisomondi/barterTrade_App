...
const TradeDetails = () => {
  ...
  const loadTrade = useCallback(
    async () => {
      ...
      try {
        ...
        setError(
          error.response?.data?.message || error.message || "Unable to load trade details."
        );
      } finally {
        setLoading(false);
      }
  ...
  const loadRatings = useCallback(async () => {
      ...
      try {
        ...
        console.error("Load trade ratings error:", error);
  ...
  const isTraderA = Boolean( currentUserId && trade?.traderAId ===  currentUserId);
  const isTraderB = Boolean(currentUserId && trade?.traderBId === currentUserId);
  const isParticipant = Boolean(isTraderA || isTraderB);
  ...
  const partnerRating = useMemo(() => {
      ...
      return ratings.find((item) => item.reviewerId ===  otherTrader.id );
  ...
  const getStageConfirmations = useCallback(
      (stage) => {
        return confirmations.filter(
          (confirmation) => confirmation.stage === stage);
      },
      [confirmations]
    );

  const getTraderConfirmation = useCallback( (stage, traderId) => {
        if (!traderId) {
          return null;
        }
        ...
        return (
          getStageConfirmations(
            ...
            (confirmation) => confirmation.userId === traderId ) || null
        );
      },
      [getStageConfirmations]
    );
  ...
  const traderBAgreementConfirmation = useMemo(() =>
        getTraderConfirmation(AGREEMENT_STAGE, trade?.traderBId),
  ...
  const currentUserAgreementConfirmation = useMemo(() => {
      ...
      return (
        agreementConfirmations.find(
          (confirmation) => confirmation.userId === currentUserId ) || null
        );
    }, [ agreementConfirmations, currentUserId,]);

  const currentUserConfirmedAgreement = Boolean(currentUserAgreementConfirmation );

  const bothTradersConfirmedAgreement = Boolean( traderAAgreementConfirmation && traderBAgreementConfirmation);
  ...
  const verificationConfirmations = useMemo(() =>
     getStageConfirmations(  VERIFICATION_STAGE ),
      [getStageConfirmations]
    );
  ...
  const traderBVerificationConfirmation = useMemo(() =>
        getTraderConfirmation( VERIFICATION_STAGE,trade?.traderBId),
      [ getTraderConfirmation,trade?.traderBId]
    );
  ...
  const currentUserVerificationConfirmation = useMemo(() => {
    ...
    }, [ verificationConfirmations, currentUserId ]);
  ...
  const currentUserHandoverConfirmation =useMemo(() => {
    ...
    }, [handoverConfirmations,currentUserId]);

  const currentUserConfirmedHandoverReadiness = Boolean(currentUserHandoverConfirmation);

  const bothTradersConfirmedHandoverReadiness =Boolean(
  ...
  const currentUserHandoverStartedConfirmation =useMemo(() => {
      ...
      return (
        handoverStartedConfirmations.find(
          (confirmation) =>
            confirmation.userId === currentUserId) || null
      );
    }, [ handoverStartedConfirmations,currentUserId]);

  const currentUserConfirmedHandoverStarted =
    Boolean( currentUserHandoverStartedConfirmation);

  const bothTradersConfirmedHandoverStarted = Boolean( traderAHandoverStartedConfirmation && traderBHandoverStartedConfirmation);
  ...
  const currentUserCompletionConfirmation = useMemo(() => {
    ...
    }, [ completionConfirmations,currentUserId,]);

  const currentUserConfirmedCompletion =Boolean( currentUserCompletionConfirmation);

  const bothTradersConfirmedCompletion =Boolean(
      traderACompletionConfirmation && traderBCompletionConfirmation);
  ...
  const handleConfirmStage = async () => {
      ...
      try {
        ...
        if ( trade.status === "PENDING" && currentUserConfirmedAgreement) {
          setConfirmationSuccess(
        ...
        if ( trade.status === "AGREED" && currentUserConfirmedVerification ) {
          setConfirmationSuccess(
        ...
        if ( trade.status === "VERIFICATION" && currentUserConfirmedHandoverReadiness) {
          setConfirmationSuccess(
  ...
  const handleConfirmHandoverStarted = async () => {
      ...
      if ( trade.status !== "READY_FOR_HANDOVER") {
        setHandoverError(
      ...
      if ( currentUserConfirmedHandoverStarted) {
        setHandoverSuccess(
  ...
  const handleConfirmCompletion =async () => {
      ...
      try {
        ...
        console.error( "Confirm trade completion error:", error);

        setActionError(error.response?.data?.message || "Unable to confirm trade completion." );
  ...
  const handleCancelTrade = async () => {
      ...
      if (
        ...
        setActionError( "This trade can no longer be cancelled at its current stage." );
        return;
      }
  ...
  const handleSubmitRating = async () => {
      ...
      if (!isParticipant) {
        setRatingError( "You are not a participant in this trade."
        );
        return;
      }
  ...
  const getHandoverStartedStatus =(userId) => {
      const confirmation = handoverStartedConfirmations.find(
          (item) =>
            item.userId ===
            userId
        );
              ...
              <div className="min-w-0">
                ...
                {!trade.dispute.outcome && (
                  <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
                    ...
                    <p className="mt-1 text-sm leading-6 text-amber-700">
                      Further trade progress is paused
                      until an administrator records the
                      final outcome.
                    </p>
                  </div>
                )}
